...
//   --setview=all     shoot EVERY preset the page exposes, in one session, to
//                     --out=<dir>/<view>.png (default /tmp/king).  king-studio
//                     takes ~90s to boot, so this saves six of those.
// Env:
...
if (setview) {
  ...
  // --setview=all drives every preset the page exposes in ONE browser session.
  // king-studio.html spends ~90s building textures, PMREMs and cube probes
  // before its first frame, so seven separate launches is seven of those.
  if (setview === 'all') {
    const names = await p.evaluate(() => Object.keys(window.KS.views));
    const dir = args.out || '/tmp/king';
    (await import('fs')).mkdirSync(dir, { recursive: true });
    if (!wantChrome) await hideChrome(p);
    const done = [];
    for (const v of names) {
      await p.evaluate((n) => window.setView(n), v);
      await p.waitForTimeout(settle);
      const f = `${dir}/${v}.png`;
      await p.screenshot({ path: f, animations: 'disabled', timeout: 180000 });
      done.push({ view: v, out: f, bytes: (await import('fs')).statSync(f).size });
    }
    console.log(JSON.stringify({ page, views: done, errors: errors.length ? errors : 'none' }));
    await browser.close();
    process.exit(0);
  }
